...
async function loadModel(app, client, config) {
  ...
  models.map(model => {
    // timestamps: true
    const addModelOptions = { timestamps: true };
    const modelOptions = originSequelize.getOptions(model.prototype);
    // 如果配置没定义，则默认 underscored: true [字段驼峰 -> 下划线]
    if ((config.define === undefined || config.define.underscored === undefined) &&
      (modelOptions === undefined || modelOptions.underscored === undefined)) {
      Object.assign(addModelOptions, { underscored: true });
    }
    originSequelize.addOptions(model.prototype, addModelOptions);
  });
